...
def create_dinov2_backbone(device):
    """Create DINOv2 backbone using transformers library"""
    # Available models:
    # - facebook/dinov2-small: 22M params
    # - facebook/dinov2-base: 87M params
    ...
    return dinov2_model.to(device), processor


class LinearProbing(nn.Module):
    def __init__(self, dinov2_model, num_classes: int):
        super().__init__()
        self.model = dinov2_model
        token_channels = dinov2_model.config.hidden_size  # 768 for DINOv2-base
...
def create_dinov2_transform(processor):
    """Create transform using DINOv2 processor"""
    def dinov2_transform(image):
        # Convert PIL image to tensor and apply DINOv2 preprocessing
...
def main(args: argparse.Namespace) -> int:
    ...
    dinov2_model, processor = create_dinov2_backbone(device)
    linear_prob = LinearProbing(dinov2_model, args.num_classes).to(device)
    
    dinov2_transform = create_dinov2_transform(processor)
    data_loader_train = create_train_dataloader(args, custom_transform=dinov2_transform)
    data_loader_val = create_val_dataloader_with_labels(args, custom_transform=dinov2_transform)
