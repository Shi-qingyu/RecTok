...
class LinearProbing(nn.Module):
    def __init__(self, model: nn.Module, num_classes: int):
        ...
        token_channels = model.encoder.token_channels // 2
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        z = self.model.tokenize(x, sampling=False)
        z = z.mean(dim=(-2, -1))
        logits = self.linear(self.norm(z))
        return logits
...
def main(args: argparse.Namespace) -> int:
    ...
    optimizer = torch.optim.SGD(trainable_params, lr=args.lr if args.lr else args.blr)
