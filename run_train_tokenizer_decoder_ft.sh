project=tokenizer_training
batch_size=128
data_path=./data/imagenet/train

model=detok_BB
token_channels=128
patch_size=16
pretrained_model_name_or_path=""
num_register_tokens=0
aux_model_type=""
aux_dec_type="transformer"
aux_input_type="noisy"
aux_target="align"
reconstruction_weight=1.0
perceptual_weight=0.5
discriminator_weight=0.5
kl_loss_weight=0.0
aux_loss_weight=0.0

epochs=100
discriminator_start_epoch=0
gamma=0.0
mask_ratio=0.0
mask_ratio_min=0.0
mask_ratio_type="fix"
vit_aux_model_size="tiny"

pretrained_exp_name="detokBB-ch128-p16-g1.0shift-m-0.10.5random-auxdinov3transformertinynoisy1.0align"
exp_name="${pretrained_exp_name}-decoderft"

GPUS_PER_NODE=${GPUS_PER_NODE:-$(nvidia-smi -L 2>/dev/null | wc -l | tr -d ' ')}
GPUS_PER_NODE=${GPUS_PER_NODE:-1}
NPROC_PER_NODE=${NPROC_PER_NODE:-$GPUS_PER_NODE}

echo "[INFO] nnodes=${WORLD_SIZE}, node_rank=${RANK}, nproc_per_node=${NPROC_PER_NODE}, master=${MASTER_ADDR}:${MASTER_PORT}"
global_batch=$(( batch_size * WORLD_SIZE * NPROC_PER_NODE ))
echo "[INFO] per-GPU batch=${batch_size}, global batch=${global_batch}"

torchrun \
  --nnodes="${WORLD_SIZE:-1}" \
  --nproc_per_node="${NPROC_PER_NODE}" \
  --node_rank="${RANK:-0}" \
  --master_addr="${MASTER_ADDR:-127.0.0.1}" \
  --master_port="${MASTER_PORT:-29501}" \
  main_reconstruction.py \
  --project "${project}" --exp_name "${exp_name}" --auto_resume \
  --batch_size "${batch_size}" --model "${model}" \
  --token_channels "${token_channels}" \
  --patch_size "${patch_size}" \
  --pretrained_model_name_or_path "${pretrained_model_name_or_path}" \
  --load_from "work_dirs/tokenizer_training/${pretrained_exp_name}/checkpoints/epoch_0199.pth" \
  --train_decoder_only \
  --blr 5e-5 --warmup_rate 0.05 \
  --gamma "${gamma}" \
  --mask_ratio "${mask_ratio}" \
  --mask_ratio_min "${mask_ratio_min}" \
  --reconstruction_weight "${reconstruction_weight}" \
  --perceptual_weight "${perceptual_weight}" \
  --discriminator_weight "${discriminator_weight}" \
  --kl_loss_weight "${kl_loss_weight}" \
  --online_eval \
  --epochs "${epochs}" --discriminator_start_epoch "${discriminator_start_epoch}" \
  --data_path "${data_path}"